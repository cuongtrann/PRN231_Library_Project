...
export const ChangeQuantityOfBook: React.FC<{
  ...
  async function increaseQuantity() {
    const url = `http://localhost:5000/api/admin/secure/increase/book/quantity/?bookId=${props.book?.id}`;
  ...
  async function decreaseQuantity() {
    const url = `http://localhost:5000/api/admin/secure/decrease/book/quantity/?bookId=${props.book?.id}`;
    const requestOptions = {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
  ...
  async function deleteBook() {
    const url = `http://localhost:5000/api/admin/secure/delete/book/?bookId=${props.book?.id}`;
    const requestOptions = {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
