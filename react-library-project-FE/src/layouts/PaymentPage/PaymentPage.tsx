...
export const PaymentPage = () => {
  ...
  useEffect(() => {
    const fetchFees = async () => {
      if (authState && authState.isAuthenticated) {
        const url = `http://localhost:5000/api/payments/search/findByUserEmail?userEmail=${authState.accessToken?.claims.sub}`;
  ...
  async function checkout() {
    ...
    const url = `http://localhost:5000/api/payments/secure/payment-intent`;
    const requestOption = {
      method: "POST",
      headers: {
    ...
    stripe
      ...
      .then(async function (result: any) {
        if (result.error) {
          ...
          const url = `http://localhost:5000/api/payments/secure/payment-complete`;
          const requestOption = {
            method: "PUT",
            headers: {
              Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
